 ...
 * part, protected by a global spinlock. The main thread waits for all the
 * threads to complete their computations, and then it prints the resulting sum.
 */
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
...
/* Define globally accessible variables and a spinlock */
static VectorData vect_data;
static pthread_spinlock_t spinlock_sum;
...
void *dot_prod(void *arg)
{
	...
	pthread_spin_lock(&spinlock_sum);
	vect_data.sum += mysum;
	pthread_spin_unlock(&spinlock_sum);
...
int main(int argc, char *argv[])
{
	...
	pthread_spin_init(&spinlock_sum, 0);
	...
	pthread_spin_destroy(&spinlock_sum);
